...
*** Functions from Trees and Tree metadata
:PROPERTIES:
:CUSTOM_ID: id-3-Functions-from-Trees-and-Tree-metadata
:END:

Every BEAST ~Tree~ object is already a ~Function~ whose $i^{\text{th}}$ element is the
age of the node with index (node number) $i$.

The ~feast.function.TraitSetAsFunction~ class can be used to represent numeric ~TraitSet~
elements as elements of a ~Function~.  For instance,
#+begin_src xml
  <nodeAges spec="feast.function.TraitSetAsFunction" traitSet="@dateTrait"/>
#+end_src
Represents such an ~Function~.

Similarly, the ~feast.function.MetadataAsFunction~ class can be used to represent
arbitrary numeric metadata included in the tree object as a ~Function~.  For instance,
#+begin_src xml
  <nodeMeta spec="feast.function.MetadataAsFunction" key="md">
    <tree spec="TreeParser" newick="(0[&md=10]:1,(1[&md=11]:1,2[&md=12]:2)3:1)4:0;"/>
  </nodeMeta> 
#+end_src
is a ~Function~ of length 5 having elements [10,11,12,NaN,NaN].  (Note that missing
values are represented in the ~Function~ with NaN.)
...
~arg~ inputs provide one or more ~Function~ objects to be referenced
...
:END:
*** Initialize RealParameters and IntegerParameters from Functions
:PROPERTIES:
:CUSTOM_ID: id-3-Initialize-RealParameters-and-IntegerParameters-from-Functions
:END:

The ~feast.parameter.RealParameterFromFunction~ and
~feast.parameter.IntegerParameterFromFunction~ classes each provide a
~Parameter~ which is **initialized** from a ~Function~.  Thus you can
do things like
...
Beware that these classes retrieve the ~Function~ values only during
initialization. **Changes in the ~Function~ values after
...
In addition, the ~timeEarlier~ and ~timeLater~ inputs can be used to
set the bounds of the parameter.  These are equivalent to ~upper~ and
~lower~ inputs of ~RealParameter~, but accept formatted times.
...
** RealParameter and IntegerParameter Operators
:PROPERTIES:
:CUSTOM_ID: id-2-RealParameter-and-IntegerParameter-Operators
:END:
...
Parameters of type ~IntegerParameter~ require special operators.  Beast has
...
~BooleanParameter~ input as the similarly-named input of the ~BlockScaleOperator~
described above.
...
** Feast Query: BEAST 2.7 Object Browser
:PROPERTIES:
:CUSTOM_ID: id-2-Feast-Query:-BEAST-2.7-Object-Browser
:END:

Feast includes a simple GUI application called ~FeastQuery~ which
can be launched using BEAST 2.7's ~AppLauncher~ utility
or by selection "launch apps" from BEAUti's File menu.

~FeastQuery~ provides information about the XML-compatible
...
[[file:figures/feastquery.png]]

The documentation provided by ~FeastQuery~ is mostly equivalent to
that provided by the BEAST 2.7 core utility, ~DocMaker~.  The main
advantages of ~FeastQuery~ are:

1. It doesn't require you to create, store and continually update a
   directory full of HTML files.
2. It doesn't require a separate web browser.
3. It has a dynamic filtering facility that lets you quickly identify
