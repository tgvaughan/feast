...
(http://maven.apache.org). Once these dependencies are in place, issue
...
This section aims to be a complete list of the features feast provides,
organised by topic.

** Importing data from files
:PROPERTIES:
:CUSTOM_ID: id-2-Importing-data-from-files
:END:
...
*** TaxonSetFromTree & TipDatesFromTree
:PROPERTIES:
:CUSTOM_ID: id-3-TaxonSetFromTree-&-TipDatesFromTree
:END:

These classs allow ~TaxonSet~ and ~TraitSet~ objects to be initialized from trees.
Like ~TraitSetFromTaxonSet~, these classes don't themselves perform any file i/o, but
...
~TipDatesFromTree~ is used similarly to create a ~TraitSet~ object
...
*** Initialize RealVectorParams from CSV/TSV files
:PROPERTIES:
:CUSTOM_ID: id-3-Initialize-RealVectorParams-from-CSV/TSV-files
:END:
...
The ~RealVectorParamFromXSV~ and ~RealVectorParamFromLabelledXSV~ classes can be
used to do this.

~RealVectorParamFromXSV~ simply initializes the parameter with values read
(in row-major order) from a rectangular portion of the table in the specified
file. For instance,
#+begin_src xml
<migrationRates spec="feast.fileio.RealVectorParamFromXSV"
...
~RealVectorParamFromLabelledXSV~ works simillarly, but assumes that the either
or both of the first row or column contain labels, and allows these labels
to be used to specify elements. For example,
#+begin_src xml
<samplingProportions spec="feast.fileio.RealVectorParamFromLabelledXSV"
...
** RealVector (array) creation and manipulation
:PROPERTIES:
:CUSTOM_ID: id-2-RealVector-(array)-creation-and-manipulation
:END:

This section describes feast classes which implement the BEAST 2
~RealVector~ interface.  A ~RealVector~ here refers to an array of
real numbers.  Many ~BEASTObject~ classes implement ~RealVector~,
including ~RealVectorParam~.

Many of the classes here take one or more ~RealVector~ objects as input,
and act to transform or combine the inputs to yield the value of the
output ~RealVector~.   This means that instances of these classes can be chained
...
*** Numeric sequences as RealVectors
:PROPERTIES:
:CUSTOM_ID: id-3-Numeric-sequences-as-RealVectors
:END:

An instance of ~feast.realvector.Sequence~ is a ~RealVector~ which contains a sequence
...
#+begin_src xml
<realVector spec="feast.realvector.Sequence" start="0" stop="10" length="101"/>
#+end_src

*** RealVector Slicing
:PROPERTIES:
:CUSTOM_ID: id-3-RealVector-Slicing
:END:

An instance of the ~feast.realvector.Slice~ class is a ~RealVector~ which
represent or more elements of another ~RealVector~.  This allows
...
of a ~RealVectorParam~:

#+begin_src xml
<distrib  spec="LogNormalDistributionModel" M="0" S="1">
    <param spec="feast.realvector.Slice" arg="@samplingProportion" index="2" count="1"/>
</distrib>
#+end_src
...
*** RealVector reversing
:PROPERTIES:
:CUSTOM_ID: id-3-RealVector-reversing
:END:

Instances of the ~feast.realvector.Reverse~ class are
~RealVector~ and ~Loggable~ which represent the elements of another
~RealVector~ but in reverse order.

For instance,
#+begin_src xml
  <reversed spec="feast.realvector.Reverse" arg="1 2 3"/>
#+end_src
behaves as a ~RealVectorParam~ with elements 3,2,1.

*** RealVector concatenation
:PROPERTIES:
:CUSTOM_ID: id-3-RealVector-concatenation
:END:

An instance of the ~feast.realvector.Concatenate~ class is a ~RealVector~
whose elements are composed of the elements of their
input ~RealVector~ objects joined together.  For example, here is a ~RealVector~
...
#+begin_src xml
<reproductiveNumber spec="feast.realvector.Concatenate">
    <arg spec="RealVectorParam" value="1.0"/>
    <arg spec="RealVectorParam" value="2.0"/>
</reproductiveNumber>
#+end_src

*** RealVector scaling
:PROPERTIES:
:CUSTOM_ID: id-3-RealVector-scaling
:END:

Instances of ~feast.realvector.Scale~ allow one ~RealVector~ to be scaled
by another:

#+begin_src xml
<scaledBirthRate spec="feast.realvector.Scale">
  <arg spec="RealVectorParam" value="1.0 2.0 3.0"/>
  <scaleBy spec="RealScalarParam" value="1.5"/>
</scaledBirthRate>
#+end_src

*** RealVector interleaving
:PROPERTIES:
:CUSTOM_ID: id-3-RealVector-interleaving
:END:

Instances of ~feast.realvector.Interleave~ represent the result of interleaving
the elements of several input ~RealVector~ objects.  For example:

#+begin_src xml
<twoDemeBirthRate spec="feast.realvector.interleave">
  <arg spec="RealVectorParam" value="1 3 5 7"/>
  <arg spec="RealVectorParam" value="2 4 6 8"/>
</twoDemeBirthRate>
#+end_src

is as a ~realvector~ with elements 1,2,3,4,5,6,7,8.

Finally, instances of ~feast.realvector.UniqueElementCount~ are
~RealVector~ objects which contain a single integer representing the
number of unique values contained in the elements of an input
~RealVector~.  For example,

#+begin_src xml
<uniqueValues spec="feast.realvector.UniqueElementCount">
    <arg spec="RealVectorParam" value="1.5 10 1.5 1.4"/>
</uniqueValues>
#+end_src

is a ~IntScalar~ with a value of 3.


*** RealVectors from Trees and Tree metadata
:PROPERTIES:
:CUSTOM_ID: id-3-RealVectors-from-Trees-and-Tree-metadata
:END:

The ~feast.realvector.TreeNodeAges~ class can be used to represent a
BEAST ~Tree~ object as a RealVector $i^{\text{th}}$ element is the
age of the node with index (node number) $i$.

The ~feast.realvector.TreeTipAges~ class is similar, but includes only
the ages of the leaf nodes in the tree.

The ~feast.realvector.TraitSetAsRealVector~ class can be used to represent numeric ~TraitSet~
elements as elements of a ~RealVector~.  For instance,
#+begin_src xml
  <nodeAges spec="feast.realvector.TraitSetAsRealVector" traitSet="@dateTrait"/>
#+end_src
represents such an ~RealVector~.

Similarly, the ~feast.realvector.MetadataAsRealVector~ class can be used to represent
arbitrary numeric metadata included in the tree object as a ~RealVector~.  For instance,
#+begin_src xml
  <nodeMeta spec="feast.function.MetadataAsRealVector" key="md">
    <tree spec="TreeParser" newick="(0[&md=10]:1,(1[&md=11]:1,2[&md=12]:2)3:1)4:0;"/>
  </nodeMeta> 
#+end_src
is a ~RealVector~ of length 5 having elements [10,11,12,NaN,NaN].  (Note that missing
values are represented in the ~RealVector~ with NaN.)
...
as a Loggable or a RealVector.  Binary operators can be applied to
RealVector (including Parameters) of different lengths as in R, with
...
Example expressions follow. (I and J are IDs of RealVectors with elements
1.0,2.0,3.0 and 5.0,10.0, respectively.)

|-------------------------------+----------------------------|
| Expression                    | Loggable/RealVector value |
|-------------------------------+----------------------------|
| ~2.5*I~                       | {2.5, 5.0, 7.5}            |
| ~I+J~                         | {6.0, 12.0, 8.0}           |
| ~exp(I[0])~                   | {2.718...}                 |
...
Note that since each ExpCalculator is a RealVector object itself, it can
be used as the input for other ExpCalculators.

As an example, here is a use of ~ExpCalculator~ to transform a
...
Notice that the expression is supplied as the ~value~ input.  The
~arg~ inputs provide one or more ~RealVector~ objects to be referenced
within the expression by their IDs.

*When used exclusively for logging*, the value caching behaviour
~ExpCalculator~ uses by default to reduce unnecessary computation can
result in the expression not being updated as necessary.  To avoid
this, set the boolean ~useCaching~ input to =false=.
...
:PROPERTIES:
:CUSTOM_ID: id-2-Parameter-initialisation
:END:
*** Initialize RealVectorParams/IntVectorParams from RealVectors/IntVectors
:PROPERTIES:
:CUSTOM_ID: id-3-Initialize-RealVectorParams/IntVectorParams-from-RealVectors/IntVectors
:END:

The ~feast.parameter.RealVectorParamFromRealVector~ and
~feast.parameter.IntVectorParamFromRealVector~ classes each provide a
~Parameter~ which is **initialized** from a ~RealVector~ or
~IntVector~.  Thus you can do things like

#+begin_src xml
<samplingRateChangeTimes spec="feast.parameter.RealVectorParamFromRealVector">
    <realVector spec="feast.expressions.ExpCalculator" value="{0,max(taxonTimes)+0.1}">
        <arg id="taxonTimes" spec="feast.function.TraitSetAsRealVector" traitSet="@dateTrait"/>
    </realVector>
...
Beware that these classes retrieve the ~RealVector~ values only during
initialization. **Changes in the ~RealVector~ values after
initialization will not be reflected in the parameter!**

*** Initializing RealVectorParams using formatted times
:PROPERTIES:
:CUSTOM_ID: id-3-Initializing-RealVectorParams-using-formatted-times
:END:
...
anywhere that a ~RealVectorParam~ is expected.  For example, to create
a ~RealVectorParam~ with the ages corresponding to the times above, use
...
#+begin_src xml
<parameter spec="RealVectorParam"
...
the ~dateFormat~ input to ~TraitSet~, and applies both to the values
of ~time~ and the value of ~mostRecentSampleTime~.  If this format
...
** RealVectorParam and IntVectorParam Operators
:PROPERTIES:
:CUSTOM_ID: id-2-RealVectorParam-and-IntVectorParam-Operators
:END:
*** Scaling subsets of RealVectorParam elements together
:PROPERTIES:
:CUSTOM_ID: id-3-Scaling-subsets-of-RealVectorParam-elements-together
:END:

It's occasionally necessary to tie multiple elements of a RealVectorParam
...
specifying respectively the ~RealVectorParam~ to scale and the ~BoolVectorParam~
...
<operator spec="feast.operators.BlockScaleOperator" parameter="@paramToScale"
    ...
    <indicator spec="BoolVector" value="true true false false" estimate="false"/>
</operator>
#+end_src
Here "@paramToScale" references a ~RealVectorParam~ with 4 elements, and the
...
#+begin_src xml
<state>
    <stateNode spec="RealVectorParam" id="paramToScale" value="1 1 2 2"/>
</state>
...
*** Operators on IntVectorParams
:PROPERTIES:
:CUSTOM_ID: id-3-Operators-on-IntVectorParams
:END:

Parameters of type ~IntVectorParam~ require special operators.  Beast has
two main operators for this type: ~IntUniformOperator~ (now deprecated in favour
of ~UniformOperator~) and ~IntRandomWalkOperator~.  Feast exptends this
collection with a pair of operators allowing for joint operation on a subset
of elements of a single ~IntVectorParam~.  The new operators are:
...
but with the addition of an input named =indicator= which takes the same
~BoolVectorParam~ input as the similarly-named input of the ~BlockScaleOperator~
described above.
...
The ~offset~ input expects a ~RealScalar~ value, so ~RealScalarParam~ objects etc. can
also be used here.
...
<populationModel spec="feast.popmodels.CompoundPopulationModel">
    ...
    <populationModel spec="ConstantPopulation" popSize="10.0"/>
    <populationModel spec="ConstantPopulation" popSize="2.0"/>
    <changeTimes spec="RealVectorParam" value="1.0 3.0"/>
</populationModel>
#+end_src
...
*** Expression coalescent population functions
:PROPERTIES:
:CUSTOM_ID: id-3-Expression-coalescent-population-functions
:END:

The ~ExpressionPopulationModel~ class allows you to specify a
population function symbolically using a mathematical expression
evaluated at runtime.

For example, here is a sinusoidally-varying population function with
...
These models can also depend on one or more ~RealVector~ objects:
#+begin_src xml
  <populationModel spec="ExpressionPopulationModel">
    exp(gamma*t)*(cos(omega*t) + 2)
    <arg id="omega" spec="RealVectorParam" value="0.3"/>
    <arg idref="gamma"/>
  </populationModel>
#+end_src
taking care that these objects have IDs defined, since these IDs are used to
refer to the objects inside of the expression.

The optional =isLog= input (boolean) can be used to indicate that the value of the
expression is the natural logarithm of the population size rather than the
population size itself.

The optional =maxTimeToConsider= input (double) can be used to change the maximum
...
For a complete description of the arithmetic expression syntax which can be used
to specify population dynamics, refer to [[ExpCalculator]].

*Beware*: ~ExpressionPopulationModel~ uses numerical integration and numerical
...
:PROPERTIES:
:CUSTOM_ID: id-2-Model-selection-and-averaging
:END:
...
of Dirichlet Process Priors (DPPs) to the elements of ~RealVector~s.
...
<distribution spec="feast.modelselect.DirichletProcessPrior" parameter="@X">
    <scaleParameter id="dppScale" spec="RealScalarParam" value="1.0"/>
...
selection in mind. It is a ~CalculationNode~ implementing ~RealVector~ which takes
one or more parameters (also ~RealVector~s) as input, together with an
~IntVector~ known as the selection index. The ~ModelSelectionParameter~
...
<deathRate id="deathRate" spec="feast.modelselect.ModelSelectionParameter">
    <parameter spec="RealVectorParam" value="0.0"/>
    <parameter id="nonZeroDeathRate" spec="RealVectorParam" value="1.0"/>
    <selectionIndices id="selectionIdx" spec="IntVectorParam" value="1" lower="0" upper="1"/>
</deathRate>
#+end_src
...
"selectionIdx" ~IntVector~.  The result is equivalent to placing a
...
In addition to this simple case, if "selectionIndices" is an ~IntVector~
...
distributions which take ~RealVector~ objects rather than
~RealVectorParam~ objects as input.
...
:PROPERTIES:
:CUSTOM_ID: id-2-Simulation-and-Testing
:END:
...
~feast.simulation.SimulatedAlignment~ also allows you to specify a known ancestral
sequence for the simulation.  To do this, supply a ~Sequence~ to the ~startingSequence~ input as
follows:
...
When used with ~startingSequence~, the optional input
~startingSequenceAge~ can be used to specify the age relative to the
...
*** Randomly initializing RealVectorParam values
:PROPERTIES:
:CUSTOM_ID: id-3-Randomly-initializing-RealVectorParam-values
:END:
...
way, ~RandomRealVectorParam~ allows you to sample new initial parameter
...
#+begin_src xml
  <init spec="feast.parameter.RandomRealVectorParam" initial="@birthRate">
      <distr spec="beast.base.spec.inference.distribution.Uniform" lower="0" upper="1.5"/>
  </init>
#+end_src

Alternatively, you can use ~RandomRealVectorParameter~ wherever BEAST expects a ~RealVectorParam~:
#+begin_src xml
  <birthRate spec="feast.parameter.RandomRealVectorParam">
    <distr spec="beast.base.spec.inference.distribution.Uniform" lower="0" upper="1.5"/>
  </birthRate>
#+end_src

*** Sequence alignment shuffling
:PROPERTIES:
:CUSTOM_ID: id-3-Sequence-alignment-shuffling
:END:
...
The ~ShuffledAlignment~ class makes this easy.  Simply replace the existing
...
object to the new ~ShuffledAlignment~ object.
...
#+begin_src xml
  <beast version="2.8">
      <run spec="feast.mapping.DensityMapper">
          ...
          <realParam spec="RealVectorParam" id="paramA" value="V_A"/>
          <range spec="RealVectorParam" id="rangeA" value="LOWER_A UPPER_A"/>
          <steps spec="IntVectorParam" value="STEPS_A"/>

          <realParam spec="RealVectorParam" id="paramB" value="V_B1 V_B2"/>
          <range spec="RealVectorParam" id="rangeB" value="LOWER_B UPPER_B"/>
          <steps spec="IntVectorParam" value="STEPS_B1 STEPS_B2"/>
...
The ~<realParam>~ inputs specify which parameters are varied, the ~<range>~ 
inputs specify the ranges over which the parameters are
...
~<logScale>~ BoolScalar element is provided, DensityMapper will
...
#+begin_src xml
  <realParam spec="RealVectorParam" id="paramA" value="V_A"/>
  <range spec="RealVectorParam" value="LOWER_A UPPER_A"/>
  <steps spec="IntVectorParam" value="STEPS_A"/>
  <logScale spec="BoolScalarParam" value="true"/>
#+end_src
...
Occasionally it's useful to be able to resurrect BEAST states from
trace and tree log files in order to perform further processing.  This
is useful, for instance, if you realise that it would have been useful
to compute additional tree statistics.  Or perhaps you'd like to
stochastically map ancestral traits onto trees belonging to the
posterior of a multi-type birth-death analysis.
The ~feast.fileio.logfileiterator.LogFileIterator~ class and its friends
allow you to do these things and more.
...
<beast version='2.0' namespace='feast.fileio.logfileiterator
                                :beast.base.spec.core.parameter'>

  <run spec="LogFileIterator">
    <logFileState spec="TraceLogFileState" logFileName="original_analysis.log">
      <logFileEntry spec="LogFileRealVectorParam" fieldName="hky.kappa">
        <fieldParameter id="kappa" spec="RealVectorParam" value="0.0"/>
      </logFileEntry>
    </logFileState>
...
~RealVectorParam~ named "kappa".  Simultaneously, we use the trees in the tree log file ~original_analysis.trees~
